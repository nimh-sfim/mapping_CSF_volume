...
SWARM_FILE='/data/SFIM_CSF_Volume/Project01/Swarm/S07.SWWARM.sh'
	...
	do
		...
		do
			...
			if [ -f ${INPUT_FILE} ]; then
					echo "export SBJ=${SBJ} SES=${SES} RUN=${RUN};sh /data/SFIM_CSF_Volume/Project01/Code/bash/S07.sh" >> ${SWARM_FILE}
			fi
		done
	done
done
